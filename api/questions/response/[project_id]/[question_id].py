...
def get_path_params(path):
    """Extract project_id and question_id from path like /api/questions/response/[project_id]/[question_id]"""
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        try:
            ...
            if result.data:
                ...
                self.send_header('Content-Type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({'error': 'Failed to save response'}).encode())
            ...
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'error': f'Failed to update response: {str(e)}'}).encode())
        return
    ...
    def do_GET(self):
        ...
        try:
            ...
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'error': f'Failed to get response: {str(e)}'}).encode())
        return
