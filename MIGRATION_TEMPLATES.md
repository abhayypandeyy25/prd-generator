# Database Migration: Templates Tables

## Issue
Templates feature is failing with error:
```
Could not find the table 'public.prd_templates' in the schema cache
```

## Solution
Run the templates migration SQL in your Supabase SQL Editor.
...
AND table_name IN ('prd_templates', 'template_sections', 'custom_questions');
```
...
Run this query to check default templates:
```sql
SELECT name, is_default, is_public FROM prd_templates ORDER BY is_default DESC, name;
```
...
Reload your PM Clarity app and try creating a new project. The template selector should now work!
